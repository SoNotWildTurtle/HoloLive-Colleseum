...
class Game(MenuMixin):
    ...
    def __init__(self, width: int = 800, height: int = 600):
        ...
        self.sound_manager = SoundManager(self.volume)
        ...
        default_map = {
            "gravity_zones": [
                {"rect": (self.width // 2 - 50, self.height - 200, 100, 50), "multiplier": 0.2},
                {"rect": (self.width // 2 + 70, self.height - 170, 80, 40), "multiplier": 2.0},
            ],
            "hazards": [
                {"type": "spike", "rect": (self.width // 3, self.height - 70, 40, 20)},
                {"type": "ice", "rect": (self.width // 2 + 80, self.height - 70, 60, 20)},
                {"type": "lava", "rect": (self.width // 2 - 100, self.height - 70, 60, 20)},
            ],
        }
        self.map_manager.add_map("Default", default_map)
        self.map_manager.set_current("Default")
        self.maps = list(self.map_manager.maps.keys())
        ...
        self.map_menu_options = list(self.map_manager.maps.keys()) + ["Back"]
        ...
        self.combat_manager = CombatManager(self.status_manager)
    ...
    def _handle_collisions(self) -> None:
        ...
        kills = self.combat_manager.handle_collisions(
            self.player,
            self.enemies,
            self.projectiles,
            self.melee_attacks,
