...
class Game:
    ...
    def __init__(self, width: int = 800, height: int = 600):
        ...
        self.character_menu_options = (
            self.characters + ["Add AI Player", "Difficulty", "Continue"]
        )
    ...
    def _draw_character_menu(self) -> None:
        options = list(self.character_menu_options)
        diff_label = f"Difficulty: {self.difficulty_levels[self.difficulty_index]}"
        diff_idx = options.index("Difficulty")
        options[diff_idx] = diff_label
        self._draw_option_menu("Select Character", options)
        ...
        while self.running:
            ...
            if self.state == "splash":
                ...
                for enemy in self.enemies:
                    proj, melee = enemy.handle_ai(self.player, now, self.hazards)
                    if proj:
                        self.projectiles.add(proj)
                        self.all_sprites.add(proj)
                    if melee:
